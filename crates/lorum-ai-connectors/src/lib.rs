...
mod tests {
    ...
    fn codex_ok_result(message_id: &str, session_id: Option<&str>, reused: bool) -> (Vec<OpenAiResponsesFrame>, CodexTransportMeta) {
        (
    ...
    fn codex_prefers_websocket_and_persists_session_state() {
        block_on(async {
            let ws = Arc::new(MockCodexWsTransport {
                responses: Mutex::new(VecDeque::from([Ok(codex_ok_result("c1", Some("ps1"), false))])),
                observed_states: Mutex::new(Vec::new()),
            });
            let sse = Arc::new(MockCodexSseTransport {
                responses: Mutex::new(VecDeque::from([Ok(codex_ok_result("unused", Some("sse"), false))])),
                observed_states: Mutex::new(Vec::new()),
            });
    ...
    fn codex_falls_back_to_sse_on_websocket_failure() {
        block_on(async {
            ...
            let sse = Arc::new(MockCodexSseTransport {
                responses: Mutex::new(VecDeque::from([Ok(codex_ok_result("c2", Some("ps2"), false))])),
                observed_states: Mutex::new(Vec::new()),
            });
            let store = Arc::new(InMemoryProviderSessionStateStore::default());
    ...
    fn codex_skips_websocket_when_state_marks_disabled() {
        block_on(async {
            let ws = Arc::new(MockCodexWsTransport {
                responses: Mutex::new(VecDeque::from([Ok(codex_ok_result("unused", Some("ws"), false))])),
                observed_states: Mutex::new(Vec::new()),
            });
            let sse = Arc::new(MockCodexSseTransport {
                responses: Mutex::new(VecDeque::from([Ok(codex_ok_result("c3", Some("ps3"), true))])),
                observed_states: Mutex::new(Vec::new()),
            });
            let store = Arc::new(InMemoryProviderSessionStateStore::default());
            store
